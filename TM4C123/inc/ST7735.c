...
// SCK    (pin 11) SDC, PA2 serial clock
...
void ST7735_OutUDec3(uint32_t n){
  Messageindex = 0;
  fillmessage3(n);
  Message[Messageindex] = 0; // terminate
  ST7735_DrawString(StX,StY,Message,StTextColor);
...
// Outputs: none
void ST7735_OutUFix1(int32_t n){
  char message[8];
  if(n>999){
      message[0] = '*';
      message[1] = '*';
...
// Outputs: none
void ST7735_OutSFix1(int32_t n){
  char message[8];
  if(n<-9999) n=-9999;
  if(n>9999)  n=9999;
  if(n<0){
...
//********ST7735_OutUFix2*****************
// Output a 32-bit number in unsigned 4-digit fixed point, 0.01 resolution
// numbers 0 to 9999 printed as " 0.00" to "99.99"
// Inputs: n  32-bit signed number
// Outputs: none
void ST7735_OutUFix2(int32_t n){
  char message[8];
  if(n>9999){
      message[0] = '*';
      message[1] = '*';
      ...
      message[4] = '*';
        ...
        if(n>=1000){  // 1000 to 9999
      message[0] = (n/1000+'0'); /* tens digit */
      n = n%1000; //the rest
    }else { // 0 to 999
      message[0] = ' '; /* n is between 0.0 and 9.9 */
    }
    message[1] = (n/100+'0'); /* ones digit */
    n = n%100; //the rest
    message[2] = '.';
    message[3] = (n/10+'0'); /* tenths digit */
    n = n%10; //the rest
    message[4] = (n+'0');    /* hundreds digit */
    }
  message[5] = 0;
  ST7735_OutString(message);
}
//********ST7735_OutSFix1_1j*****************
// Output a 16-bit number in signed 2-digit fixed point, 0.1 resolution
// numbers -99 to 99 printed as "-9.9j" to "+9.9j"
// Inputs: n  32-bit signed number
// Outputs: none
void ST7735_OutSFix1_1j(int32_t n){
  char message[8];
  if(n<-99) n=-99;
  if(n>99)  n=99;
  if(n<0){
...
#define jmin    40
#define jmin2   109
#define jscale  (159-jmin)
#define jscale2 (159-jmin2)
int32_t Ymax,Ymin,X;    // X goes from 0 to 127
int32_t Ymax2,Ymin2;    // second point scale
int32_t Yrange,Yrange2; //YrangeDiv2;
...
// Inputs: ymin and ymax are range of the plot
// Outputs: none
void ST7735_PlotClear2(int32_t ymin, int32_t ymax,int32_t ymin2, int32_t ymax2){
  ST7735_FillRect(0, jmin, 128, 128, PLOT_BACKGROUND);
  if(ymax>ymin){
    Ymax = ymax;
    Ymin = ymin;
    Yrange = ymax-ymin;
...
// This routine does not clear the display
// Inputs: ymin and ymax are range of the plot
// Outputs: none
void ST7735_PlotRescale2(int32_t ymin, int32_t ymax,int32_t ymin2, int32_t ymax2){
  if(ymax>ymin){
    Ymax = ymax;
    Ymin = ymin;
    Yrange = ymax-ymin;
...
//         y2 is the y coordinate of the second point plotted
// Outputs: none
void ST7735_PlotPoints2(int32_t y1,int32_t y2){int32_t j;
  if(y1<Ymin) y1=Ymin;
  if(y1>Ymax) y1=Ymax;
  // X goes from 0 to 127
  // j goes from 159 to jmin
  // y=Ymax maps to j=jmin
  // y=Ymin maps to j=159
  j = jmin+(jscale*(Ymax-y1))/Yrange;
  if(j<jmin) j = jmin;
  if(j>158) j = 158;
  ST7735_DrawPixel(X, j, PLOT_COLOR1);
  ST7735_DrawPixel(X, j+1, PLOT_COLOR1);
  if(y2<Ymin2) y2=Ymin2;
  if(y2>Ymax2) y2=Ymax2;
  // j2 goes from 159 to jmin2
  // y=Ymax2 maps to j=jmin2
  // y=Ymin2 maps to j=159
  j = jmin2+(jscale2*(Ymax2-y2))/Yrange2;
  if(j<jmin2) j = jmin2;
  if(j>159) j = 159;
  ST7735_DrawPixel(X, j, PLOT_COLOR2);
}
